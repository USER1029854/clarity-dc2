...
| `verify_bytecode.py` | runs the **real deployed treasury validator `fcc43cd5`** in the UPLC CEK machine: ACCEPTS the drain on a GAT burn, REJECTS without it (negative controls) | `pip install uplc==1.3.3 cbor2==5.6.5` |
...
| `treasury_fcc43cd5.cbor` | the deployed treasury validator bytecode (PlutusV2), fetched from chain | — |
...
python3 verify_bytecode.py   # real treasury bytecode: GAT burn -> releases 780M CLARITY
...
- **Recyclable weight (the bug):** the deployed stake validator `08921c4b` accepted **37 of 38**
  real delock (redeemer 2) txs that removed a position's lock while leaving stake weight
  unchanged (`../evidence/delock_analysis.json`). `verify_onchain.py` replays one
  (`6a2a55a2`, 5,000,000 CLARITY freed, weight unchanged) and shows 253M of live unbacked weight.
...
fork-style emulator seeded with pinned real state (block 13,914,607 / epoch 654), with the drain
gate executed on the real deployed bytecode and the weight bug proven by real mainnet txs. The
recycle loop composes primitives each independently confirmed on-chain; assembling them into a
single wallet run is what the emulator does. See `../VERIFICATION.md` for the full audit trail.
